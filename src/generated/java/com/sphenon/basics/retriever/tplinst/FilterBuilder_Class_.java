...
public class FilterBuilder_Class_ implements GenericFilterBuilder<Class> {

    public FilterBuilder_Class_ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Class.class)) : target_type);
    }
    ...
    protected GenericFilter<Class> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Class> filter = Factory_Filter_Class_.construct(context);

        if ((filter instanceof FilterByValue_Class_) == false || operator == null || operator.matches("[=]") == false) {
                              ...
                              if (value instanceof Class) {
                                  ((FilterByValue_Class_) filter).setValue(context, (Class) value);
                              }
    ...
    protected Getter<GenericFilter<Class>> getter;
    protected Setter<GenericFilter<Class>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Class>> getter, Setter<GenericFilter<Class>> setter, Creator<GenericFilter<Class>> creator) {
        this.getter = getter;
        this.setter = setter;
