...
public class FilterBuilder_Byte_ implements GenericFilterBuilder<Byte> {

    public FilterBuilder_Byte_ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Byte.class)) : target_type);
    }
    ...
    protected GenericFilter<Byte> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Byte> filter = Factory_Filter_Byte_.construct(context);

        if ((filter instanceof FilterByRange_byte_) == false || operator == null || operator.matches("[=<>≤≥]") == false) {
                                 ...
                                 if (value instanceof Long) {
                                     ((FilterByRange_byte_) filter).setMinimum(context, (byte) (long) (Long) value);
                                 } else if (value instanceof String) {
                                     ((FilterByRange_byte_) filter).setMinimum(context, (String) value);
                                 } else {
                                 ...
                                 } ((FilterByRange_byte_) filter).setIncludeMinimum(context, operator.equals("≥") ? true : false);
                             } if (operator.matches("[<≤]")) {
                                 if (value instanceof Long) {
                                     ((FilterByRange_byte_) filter).setMaximum(context, (byte) (long) (Long) value);
                                 } else if (value instanceof String) {
                                     ((FilterByRange_byte_) filter).setMaximum(context, (String) value);
                                 } else {
                                     InvalidQueryExpression.createAndThrow(context, "Cannot apply condition '%(operator)' '%(value)' (of type '%(valuetype)') to filter of type '%(type)'", "operator", operator, "value", value, "valuetype", value.getClass().getName(), "type", filter.getClass().getName());
                                 } ((FilterByRange_byte_) filter).setIncludeMaximum(context, operator.equals("≤") ? true : false);
                             } if (operator.equals("=")) {
                                 if (value instanceof Long) {
                                     ((FilterByRange_byte_) filter).setMinimum(context, (byte) (long) (Long) value);
                                     ((FilterByRange_byte_) filter).setMaximum(context, (byte) (long) (Long) value);
                                 ...
                                 ((FilterByRange_byte_) filter).setIncludeMinimum(context, true);
                                 ((FilterByRange_byte_) filter).setIncludeMaximum(context, true);
                             }
    ...
    protected Getter<GenericFilter<Byte>> getter;
    protected Setter<GenericFilter<Byte>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Byte>> getter, Setter<GenericFilter<Byte>> setter, Creator<GenericFilter<Byte>> creator) {
        this.getter = getter;
        this.setter = setter;
