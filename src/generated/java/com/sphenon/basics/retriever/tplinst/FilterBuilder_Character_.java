...
public class FilterBuilder_Character_ implements GenericFilterBuilder<Character> {

    public FilterBuilder_Character_ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Character.class)) : target_type);
    }
    ...
    protected GenericFilter<Character> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Character> filter = Factory_Filter_Character_.construct(context);

        if ((filter instanceof FilterByExpression_char_) == false || operator == null || operator.matches("[=~]") == false) {
                                  ...
                                  ((FilterByExpression_char_) filter).setExpression(context, (operator.matches("~") ? "~" : "") + value);
    ...
    protected Getter<GenericFilter<Character>> getter;
    protected Setter<GenericFilter<Character>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Character>> getter, Setter<GenericFilter<Character>> setter, Creator<GenericFilter<Character>> creator) {
        this.getter = getter;
        this.setter = setter;
