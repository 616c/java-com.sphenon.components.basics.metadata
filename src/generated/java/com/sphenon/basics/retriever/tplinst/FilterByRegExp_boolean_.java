...
public class FilterByRegExp_boolean_ extends GenericFilterBase<Boolean> implements FilterByExpression_boolean_ {
    ...
    public FilterByRegExp_boolean_ (CallContext context) {
        super(context, target_type == null ? (target_type = TypeManager.get(context, Boolean.class)) : target_type);
    }

    public FilterByRegExp_boolean_ (CallContext context, String regexp) throws PatternSyntaxException {
        this(context);
    ...
    public boolean matches (CallContext context, boolean object) {
        boolean instance = new Boolean(object);
        return ( this.regexp != null
                 ? (    this.regexp.length() == 0
                     || this.regexp_pattern == null
                     || (new Boolean(object) != null && regexp_pattern.matcher(new Boolean(instance).toString() ).matches())
                   ) :
                 this.value != null
                 ? (new Boolean(object) != null && this.value.equals(new Boolean(instance).toString() )
                   ) :
                 true);
    }
