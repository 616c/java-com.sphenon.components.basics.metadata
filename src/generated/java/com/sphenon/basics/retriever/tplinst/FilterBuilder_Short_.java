...
public class FilterBuilder_Short_ implements GenericFilterBuilder<Short> {

    public FilterBuilder_Short_ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Short.class)) : target_type);
    }
    ...
    protected GenericFilter<Short> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Short> filter = Factory_Filter_Short_.construct(context);

        if ((filter instanceof FilterByRange_short_) == false || operator == null || operator.matches("[=<>≤≥]") == false) {
                                  ...
                                  if (value instanceof Long) {
                                      ((FilterByRange_short_) filter).setMinimum(context, (short) (long) (Long) value);
                                  } else if (value instanceof String) {
                                      ((FilterByRange_short_) filter).setMinimum(context, (String) value);
                                  } else {
                                  ...
                                  } ((FilterByRange_short_) filter).setIncludeMinimum(context, operator.equals("≥") ? true : false);
                              } if (operator.matches("[<≤]")) {
                                  if (value instanceof Long) {
                                      ((FilterByRange_short_) filter).setMaximum(context, (short) (long) (Long) value);
                                  } else if (value instanceof String) {
                                      ((FilterByRange_short_) filter).setMaximum(context, (String) value);
                                  } else {
                                      InvalidQueryExpression.createAndThrow(context, "Cannot apply condition '%(operator)' '%(value)' (of type '%(valuetype)') to filter of type '%(type)'", "operator", operator, "value", value, "valuetype", value.getClass().getName(), "type", filter.getClass().getName());
                                  } ((FilterByRange_short_) filter).setIncludeMaximum(context, operator.equals("≤") ? true : false);
                              } if (operator.equals("=")) {
                                  if (value instanceof Long) {
                                      ((FilterByRange_short_) filter).setMinimum(context, (short) (long) (Long) value);
                                      ((FilterByRange_short_) filter).setMaximum(context, (short) (long) (Long) value);
                                  ...
                                  ((FilterByRange_short_) filter).setIncludeMinimum(context, true);
                                  ((FilterByRange_short_) filter).setIncludeMaximum(context, true);
                              }
    ...
    protected Getter<GenericFilter<Short>> getter;
    protected Setter<GenericFilter<Short>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Short>> getter, Setter<GenericFilter<Short>> setter, Creator<GenericFilter<Short>> creator) {
        this.getter = getter;
        this.setter = setter;
