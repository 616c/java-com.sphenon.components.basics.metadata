...
public class FilterBuilder_Date_ implements GenericFilterBuilder<Date> {

    public FilterBuilder_Date_ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Date.class)) : target_type);
    }
    ...
    protected GenericFilter<Date> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Date> filter = Factory_Filter_Date_.construct(context);

        if ((filter instanceof FilterByRange_Date_) == false || operator == null || operator.matches("[=<>≤≥]") == false) {
                             ...
                             if (value instanceof Date) {
                                     ((FilterByRange_Date_) filter).setMinimum(context, (Date) value);
                                 } else if (value instanceof String) {
                                     ((FilterByRange_Date_) filter).setMinimum(context, (String) value);
                                 } else {
                                 ...
                                 ((FilterByRange_Date_) filter).setIncludeMinimum(context, operator.equals("≥") ? true : false);
                             }
                             if (operator.matches("[<≤]")) {
                                 if (value instanceof Date) {
                                     ((FilterByRange_Date_) filter).setMaximum(context, (Date) value);
                                 } else if (value instanceof String) {
                                     ((FilterByRange_Date_) filter).setMaximum(context, (String) value);
                                 } else {
                                     InvalidQueryExpression.createAndThrow(context, "Cannot apply condition '%(operator)' '%(value)' (of type '%(valuetype)') to filter of type '%(type)'", "operator", operator, "value", value, "valuetype", value.getClass().getName(), "type", filter.getClass().getName());
                                 }
                                 ((FilterByRange_Date_) filter).setIncludeMaximum(context, operator.equals("≤") ? true : false);
                             }
                             if (operator.equals("=")) {
                                 if (value instanceof Date) {
                                     ((FilterByRange_Date_) filter).setMinimum(context, (Date) value);
                                 ...
                                 ((FilterByRange_Date_) filter).setIncludeMinimum(context, true);
                                 ((FilterByRange_Date_) filter).setIncludeMaximum(context, true);
                             }
    ...
    protected Getter<GenericFilter<Date>> getter;
    protected Setter<GenericFilter<Date>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Date>> getter, Setter<GenericFilter<Date>> setter, Creator<GenericFilter<Date>> creator) {
        this.getter = getter;
        this.setter = setter;
