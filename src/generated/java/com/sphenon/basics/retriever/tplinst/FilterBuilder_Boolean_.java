...
public class FilterBuilder_Boolean_ implements GenericFilterBuilder<Boolean> {

    public FilterBuilder_Boolean_ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Boolean.class)) : target_type);
    }
    ...
    protected GenericFilter<Boolean> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Boolean> filter = Factory_Filter_Boolean_.construct(context);

        if ((filter instanceof FilterByValue_boolean_) == false || operator == null || operator.matches("[=]") == false) {
                                    ...
                                    ((FilterByValue_boolean_) filter).setValue(context, (Boolean) value);
                                } else if (value instanceof String) {
                                    ((FilterByValue_boolean_) filter).setValue(context, (String) value);
                                } else {
    ...
    protected Getter<GenericFilter<Boolean>> getter;
    protected Setter<GenericFilter<Boolean>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Boolean>> getter, Setter<GenericFilter<Boolean>> setter, Creator<GenericFilter<Boolean>> creator) {
        this.getter = getter;
        this.setter = setter;
