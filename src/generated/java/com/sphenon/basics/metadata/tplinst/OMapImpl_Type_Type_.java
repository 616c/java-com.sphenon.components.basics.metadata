...
public class OMapImpl_Type_Type_
  implements OMap_Type_Type_
{
    protected java.util.Hashtable map;

    protected class Entry {
        private Type                     actual_item;
        private Type                     cached_item;
        private java.util.Vector             child_entries;
        private java.util.Vector             parent_entries;
        private OMapImpl_Type_Type_ omap;
        private Type                    index;
        private int                          path_length;
        private Type                    cached_index;

        public Entry (CallContext context, OMapImpl_Type_Type_ omap, Type index) {
            this.omap = omap;
            this.index = index;
        ...
        public void setItem(CallContext context, Type item) {
        ...
        public Type getItem(CallContext context) {
        ...
        public void update (CallContext context) {
            ...
            for (int i=0; i < this.parent_entries.size(); i++) {
                ...
                Type parent_item = parent_entry.getItem(context);
                if (parent_item != null) {
    ...
    public OMapImpl_Type_Type_ (CallContext context)
    {
        map = new java.util.Hashtable ();
    }

    public OMapImpl_Type_Type_ (CallContext context, java.util.Hashtable map )
    {
        this.map = map;
    }
    ...
    {
        Type item = this.tryGet(context, index);
    ...
    public void     set     (CallContext context, Type index, Type item)
    {
        Entry entry = this._getEntry(context, index);
        entry.setItem(context, item);
    }

    public void     add     (CallContext context, Type index, Type item) throws AlreadyExists
    {
        Entry entry = this._getEntry(context, index);
    ...
    public void     replace (CallContext context, Type index, Type item) throws DoesNotExist
    {
        Entry entry = this._getEntry(context, index);
