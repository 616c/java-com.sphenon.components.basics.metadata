...
public class OOMapImpl<T> implements OOMap<T> {
    static protected long notification_level;
    static public    long adjustNotificationLevel(long new_level) { long old_level = notification_level; notification_level = new_level; return old_level; }
    static public    long getNotificationLevel() { return notification_level; }
    static { notification_level = NotificationLocationContext.getLevel(RootContext.getInitialisationContext(), "com.sphenon.basics.metadata.classes.OOMapImpl"); };
    ...
    protected class Entry {
        ...
        public Entry(CallContext context, OOMapImpl<T> oomap, Type index1, Type index2) {
            CustomaryContext cc = (((notification_level & Notifier.SELF_DIAGNOSTICS) != 0) ? CustomaryContext.create((Context) context) : null);

            if ((notification_level & Notifier.SELF_DIAGNOSTICS) != 0) { cc.sendTrace(context, Notifier.SELF_DIAGNOSTICS, "new Entry in OOMapImpl<T> : (%(index1),%(index2))", "index1", index1, "index2", index2); }
        ...
        public void update (CallContext context) {
            ...
            if ((notification_level & Notifier.SELF_DIAGNOSTICS) != 0) { cc.sendTrace(context, Notifier.SELF_DIAGNOSTICS, "update Entry in OOMapImpl<T>: current path 1 length %(pathlen1), current path 2 length %(pathlen2)", "pathlen1", t.s(this.path_length1), "pathlen2", t.s(this.path_length2)); }
    ...
    public OOMapImpl(CallContext context) {
        map1 = new HashMap();
    }

    public OOMapImpl(CallContext context, Map map1) {
        this.map1 = map1;
    }
    ...
    private Entry _getEntry (CallContext context, Type index1, Type index2) {
        ...
        if ((this.notification_level & Notifier.SELF_DIAGNOSTICS) != 0) { cc.sendTrace(context, Notifier.SELF_DIAGNOSTICS, "get Entry in OOMapImpl<T> : (%(index1),%(index2))", "index1", index1, "index2", index2); }
        Map map2 = (Map) map1.get(index1);
        if (map2 == null) {
            if ((this.notification_level & Notifier.SELF_DIAGNOSTICS) != 0) { cc.sendTrace(context, Notifier.SELF_DIAGNOSTICS, "get Entry in OOMapImpl<T> : (%(index1),%(index2)): not in map1, creating new Map...", "index1", index1, "index2", index2); }
            map2 = new HashMap();
            map1.put(index1, map2);
        }
        Entry entry = (Entry) map2.get(index2);
        if (entry == null) {
            if ((this.notification_level & Notifier.SELF_DIAGNOSTICS) != 0) { cc.sendTrace(context, Notifier.SELF_DIAGNOSTICS, "get Entry in OOMapImpl<T> : (%(index1),%(index2)): not in map2, creating new Entry...", "index1", index1, "index2", index2); }
            entry = new Entry (context, this, index1, index2);
            map2.put(index2, entry);
        }
        if ((this.notification_level & Notifier.SELF_DIAGNOSTICS) != 0) { cc.sendTrace(context, Notifier.SELF_DIAGNOSTICS, "got Entry in OOMapImpl<T> : (%(index1),%(index2)): %(entry)", "index1", index1, "index2", index2, "entry", entry); }
        return entry;
    }

    public T get (CallContext context, Type index1, Type index2) throws DoesNotExist {
        T item = this.tryGet(context, index1, index2);
        if (item == null) DoesNotExist.createAndThrow(context);
        return item;
    }

    public T tryGet  (CallContext context, Type index1, Type index2) {
    ...
    public void     set     (CallContext context, Type index1, Type index2, T item) {
        Entry entry = this._getEntry(context, index1, index2);
        entry.setItem(context, item);
    }

    public void     add     (CallContext context, Type index1, Type index2, T item) throws AlreadyExists {
        Entry entry = this._getEntry(context, index1, index2);
        if (entry.isActualEntry()) AlreadyExists.createAndThrow (context);
        entry.setItem(context, item);
    }

    public void     replace (CallContext context, Type index1, Type index2, T item) throws DoesNotExist {
        Entry entry = this._getEntry(context, index1, index2);
